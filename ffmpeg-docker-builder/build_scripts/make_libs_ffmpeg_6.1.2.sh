...
build_libx264() {
  ...
  sed -i -e 's/-lx264/-lx264-nimble/' x264.pc
  make -j 4
  make install
  cd ..
}
...
build_libfdkaac() {
  ...
  sed -i -e 's/-lfdk-aac/-lfdk-aac-nimble/' fdk-aac.pc
  make -j 4
  make install
  cd ..
}
...
build_libzvbi() {
  if [ ! -d zvbi ]; then
    git clone https://github.com/zapping-vbi/zvbi.git zvbi
  fi
  ...
  ./autogen.sh
  ./configure --prefix=$BUILD_DIR --enable-shared=no "CFLAGS=-g -O2 -fPIC"
  make -j4
  make install
  popd
}
...
build_ffmpeg() {
  ...
  if [ $build_with_nvenc == 1 ]; then
      ...
      git checkout n12.0.16.1 # ffmpeg doesn't build with oldest nvenc, it is require cuCtxGetCurrent
  ...
  if [ ! -d ffmpeg-$FFMPEG_VERSION ]; then
      ...
      if [ "$FFMPEG_VERSION" == "6.1.2" ]; then
        # IMPORTANT NOTE: follow how_2_upgrade_ffmpeg.txt to make sure patches applied correctly and still valid
        ...
        md5sum -c qsvenc.c.6.1.2.original.md5
        patch  ffmpeg-6.1.2/libavcodec/qsvenc.c qsvenc.c.6.1.2.FrameType.patch
        ...
        md5sum -c vf_overlay_qsv.c.6.1.2.original.md5
        patch  ffmpeg-6.1.2/libavfilter/vf_overlay_qsv.c vf_overlay_qsv.c.6.1.2.frame_props.patch
      fi
  fi
  ...
  # for ubuntu 24.04 - apt-get install libfreetype-dev libharfbuzz-dev libmfx-dev libspeex-dev libva-dev nvidia-cuda-dev nvidia-cuda-toolkit

  if [ $build_with_nvenc == 1 ]; then
      FFMPEG_ENCODERS="aac,png,mjpeg,h264_nvenc,hevc_nvenc,av1_nvenc"
      FFMPEG_DECODERS="aac,pcm_alaw,pcm_mulaw,mp2,h264,mpeg2video,mp3,png,mjpeg,tiff,gif,bmp,libspeex,hevc,vp8,vp9,ac3,h264_cuvid,hevc_cuvid,mpeg2_cuvid,vp8_cuvid,vp9_cuvid,av1_cuvid"
  ...
  FFMPEG_ENCODERS+=",libopus,ac3,eac3,libx264"
  ...
  FFMPEG_DECODERS+=",libzvbi_teletext"
  ...
  PKG_CONFIG_PATH=$BUILD_DIR/lib/pkgconfig ./configure \
    ...
    --disable-parsers    \
    --enable-parser=h264,hevc,aac,opus,dvbsub,gif \
    --disable-bsfs       \
    --disable-protocols  \
