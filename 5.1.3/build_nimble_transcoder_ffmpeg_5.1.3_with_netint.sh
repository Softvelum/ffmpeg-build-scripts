...
build_ffmpeg() {
  ...
  build_with_netint_logan=1
  build_with_netint_quadra=0
  ...
  if [ ! -d ffmpeg-$FFMPEG_VERSION ]; then
      ...
      if [ "$FFMPEG_VERSION" == "5.1.3" ]; then
        ...
        if [ $build_with_netint_logan == 1 ]; then
            if [ ! -d T4xx-Release-v3.3.7 ]; then
                if [ ! -f T4xx-Release-v3.3.7.zip ]; then
                    echo 'download T4xx-Release-v3.3.7.zip from NETINT documentation portal'
                    # e.g. wget https://xxx.netint.ca/xxx/T4xx-Release-v3.3.7.zip
                fi

                unzip T4xx-Release-v3.3.7.zip -d T4xx-Release-v3.3.7
                pushd T4xx-Release-v3.3.7
                tar -xzvf Codensity_T4XX_Software_Release_V3.3.7.tar.gz
                popd
            fi

            if [ ! -d libxcoder_logan ]; then
               cp -r T4xx-Release-v3.3.7/release/libxcoder_logan .
               pushd libxcoder_logan
               bash build.sh
               sudo ldconfig   
               popd
            fi

            cp T4xx-Release-v3.3.7/release/FFmpeg-n5.1.2_t4xx_patch ./ffmpeg-$FFMPEG_VERSION
            pushd ./ffmpeg-$FFMPEG_VERSION

            # ignore fail on VERSION file patching
            patch -t -p 1 < FFmpeg-n5.1.2_t4xx_patch || true
            popd
        fi
        
        if [ $build_with_netint_quadra == 1 ]; then
            if [ ! -d Quadra_V4.7.0 ]; then
                if [ ! -f Quadra_V4.7.0.zip ]; then
                    echo 'download Quadra_V4.7.0.zip from NETINT documentation portal'
                    # e.g. wget https://xxx.netint.ca/xxx/Quadra_V4.7.0.zip
                fi

                unzip Quadra_V4.7.0.zip
                cp Quadra_V4.7.0/Quadra_SW_V*/FFmpeg-*.diff ./ffmpeg-$FFMPEG_VERSION
                pushd ./ffmpeg-$FFMPEG_VERSION

                patch -t -p 1 < FFmpeg-n5.0_netint_v4.7.0_RC4.diff
                popd
          
                if [ ! -d libxcoder ]; then
                    cp -r Quadra_V4.7.0/Quadra_SW_V*/libxcoder ./
                    pushd libxcoder
                    bash build.sh
                    sudo ldconfig
                    popd
  ...
  if [ $build_with_netint_logan == 1 ]; then
      FFMPEG_ENCODERS+=",h264_ni_logan,h265_ni_logan"
      FFMPEG_NETINT_OPTIONS="--enable-libxcoder_logan --enable-ni_logan"
  fi

  if [ $build_with_netint_quadra == 1 ]; then
      FFMPEG_ENCODERS+=",h264_ni_quadra,h265_ni_quadra,av1_ni_quadra"
      FFMPEG_NETINT_OPTIONS="--enable-ni_quadra"
  fi
